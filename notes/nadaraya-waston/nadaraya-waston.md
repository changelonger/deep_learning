...
# 在 Python 中，_ 通常被用作一个占位符，用来表示你不关心的值或变量。
# torch.sort() 函数返回两个值：
# 排序后的张量（排序后的数据），
# 对应的索引（原始数据中每个元素的新位置）。
x_train
```
    ...
    tensor([0.1233, 0.1535, 0.5983, 0.6211, 0.6825, 0.7179, 0.8085, 1.0634, 1.4574,
            1.5748, 1.6310, 2.0524, 2.0708, 2.0935, 2.1027, 2.1056, 2.2051, 2.2566,
            2.3234, 2.4514, 2.5953, 2.8122, 2.8199, 2.8761, 2.8768, 2.8827, 2.9115,
            3.1253, 3.3449, 3.4512, 3.5828, 3.7590, 3.9095, 3.9285, 4.0253, 4.0262,
            4.0350, 4.0552, 4.1091, 4.3386, 4.3729, 4.4183, 4.4184, 4.4703, 4.5106,
            4.5343, 4.7043, 4.8587, 4.9005, 4.9632])
...
class NWKernelRegression(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        # repeat_interleave(keys.shape[1]) 表示对 queries 张量中的每个元素重复 keys.shape[1] 次
